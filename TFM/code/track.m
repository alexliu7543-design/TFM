function trks = track(xyzt,maxdisp,varargin)
...
% This is a function used to compare two strings and is not case sensitive.
% Here, it checks whether each element in varargin is equal to the string 'min_track_length'.
if inputExist
...
% point apart.
trackable_time_inds=find(xyzt(2:end,sz(2))-xyzt(1:end-1,sz(2))==1); % 时间分割项
trackable_time_inds=[trackable_time_inds;trackable_time_inds(end)+1];
trackable_times=xyzt(trackable_time_inds,sz(2)); % 对应的具体的时间
...
for k=1:length(trackable_times)
    ...
    pts1=xyzt(xyzt(:,sz(2))==trackable_times(k),1:dim); % 位置信息的两列矩阵
    pts2=xyzt(xyzt(:,sz(2))==trackable_times(k)+1,1:dim);
    ...
    waitbar(k/(length(trackable_times)-1),f,'% of the way through time steps (step 1/2)');
end
