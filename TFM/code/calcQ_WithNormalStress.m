function Q = calcQ_WithNormalStress(z,h,EM,nu,nr,dx,D)
    ...
    %
    %   Chang Xu 2022-12-28
    %     - Added Normal stress consideration
    %     - Without tracking the particles in 3D for vertical displacement
    %
    % INPUTS
    ...
    % % % Consider with the normal stress
    % Construct the New Matrix "B_new" with which we can get the zero
    % position $d\hat{u}^0$ with the displacement field at z = observation
    % plane and normal stress on the top plane
    
    Bnew(1,1).M = (exp( k * (z) )./2).*Mz(1,1).M;
    Bnew(1,2).M = (exp( k * (z) )./2).*Mz(1,2).M;
    Bnew(1,3).M = (exp( k * (z) )./2).*Mz(1,3).M;
    
    Bnew(2,1).M = (exp( k * (z) )./2).*Mz(2,1).M;
    Bnew(2,2).M = (exp( k * (z) )./2).*Mz(2,2).M;
    Bnew(2,3).M = (exp( k * (z) )./2).*Mz(2,3).M;
    
%     Bnew(3,1).M = (exp( k * (h) )./2).*EM/((1+nu)*(1-2*nu)).*((1-nu).*Nh(3,1).N + complex(0,a).*nu*Mh(1,1).M + complex(0,b).*nu*Mh(2,1).M);
%     Bnew(3,2).M = (exp( k * (h) )./2).*EM/((1+nu)*(1-2*nu)).*((1-nu).*Nh(3,2).N + complex(0,a).*nu*Mh(1,2).M + complex(0,b).*nu*Mh(2,2).M);
%     Bnew(3,3).M = (exp( k * (h) )./2).*EM/((1+nu)*(1-2*nu)).*((1-nu).*Nh(3,3).N + complex(0,a).*nu*Mh(1,3).M + complex(0,b).*nu*Mh(2,3).M);
%     
    
    Bnew(3,1).M = (exp( k * (h) )./2).*EM/((1+nu)*(1-2*nu)).*((1-nu).*Nh(3,1).N + complex(0,nu).*a.*Mh(1,1).M + complex(0,nu).*b.*Mh(2,1).M);
    Bnew(3,2).M = (exp( k * (h) )./2).*EM/((1+nu)*(1-2*nu)).*((1-nu).*Nh(3,2).N + complex(0,nu).*a.*Mh(1,2).M + complex(0,nu).*b.*Mh(2,2).M);
    Bnew(3,3).M = (exp( k * (h) )./2).*EM/((1+nu)*(1-2*nu)).*((1-nu).*Nh(3,3).N + complex(0,nu).*a.*Mh(1,3).M + complex(0,nu).*b.*Mh(2,3).M);
    
   

    % Construct the inverse of M
    %   Determinant of Bnew
    detBnew = Bnew(1,1).M.* (Bnew(2,2).M.*Bnew(3,3).M - Bnew(3,2).M.*Bnew(2,3).M) ...
          - Bnew(1,2).M.* (Bnew(2,1).M.*Bnew(3,3).M - Bnew(3,1).M.*Bnew(2,3).M) ...
          + Bnew(1,3).M.* (Bnew(2,1).M.*Bnew(3,2).M - Bnew(3,1).M.*Bnew(2,2).M);
    %   Inverse of the determinant
    invdetBnew = 1./detBnew;
    %   Inverse of M
    Bnewinv(1,1).M = invdetBnew .* (Bnew(2,2).M.*Bnew(3,3).M - Bnew(2,3).M.*Bnew(3,2).M);
    Bnewinv(1,2).M = invdetBnew .* (Bnew(1,3).M.*Bnew(3,2).M - Bnew(1,2).M.*Bnew(3,3).M);
    Bnewinv(1,3).M = invdetBnew .* (Bnew(1,2).M.*Bnew(2,3).M - Bnew(1,3).M.*Bnew(2,2).M);
    Bnewinv(2,1).M = invdetBnew .* (Bnew(2,3).M.*Bnew(3,1).M - Bnew(2,1).M.*Bnew(3,3).M);
    Bnewinv(2,2).M = invdetBnew .* (Bnew(1,1).M.*Bnew(3,3).M - Bnew(1,3).M.*Bnew(3,1).M);
    Bnewinv(2,3).M = invdetBnew .* (Bnew(1,3).M.*Bnew(2,1).M - Bnew(1,1).M.*Bnew(2,3).M);
    Bnewinv(3,1).M = invdetBnew .* (Bnew(2,1).M.*Bnew(3,2).M - Bnew(2,2).M.*Bnew(3,1).M);
    Bnewinv(3,2).M = invdetBnew .* (Bnew(1,2).M.*Bnew(3,1).M - Bnew(1,1).M.*Bnew(3,2).M);
    Bnewinv(3,3).M = invdetBnew .* (Bnew(1,1).M.*Bnew(2,2).M - Bnew(1,2).M.*Bnew(2,1).M);

    
    % Construct matrix Q = <stuff> * Ph * Mzinv
    for i=1:3
        for j=1:3
            Q(i,j).Q = Ph(i,1).P.*Bnewinv(1,j).M + Ph(i,2).P.*Bnewinv(2,j).M + Ph(i,3).P.*Bnewinv(3,j).M;
            ...
            Q(i,j).Q = (exp( k * (h) )./2).* Q(i,j).Q; % exp(-kz)/2  already put into Bnew
        end
    end
    ...
    % Consider the normal stress at top
    Q(3,3).Q(ncenter,ncenter) = 1;
